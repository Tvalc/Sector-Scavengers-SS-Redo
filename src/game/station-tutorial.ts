/**
 * Station Tutorial Overlay
 *
 * Handles tutorial overlay for station screens (hub, salvage-market, void-communion, hardware, cryo, modules).
 */
...
import type { StationScreen } from './types';
...
/**
 * Check if the current screen is a panel screen (salvage-market, void-communion, hardware, cryo, modules).
 */
function checkIsPanelScreen(screen: StationScreen): boolean {
  return ['salvage-market', 'void-communion', 'hardware', 'cryo', 'modules'].includes(screen);
}
...
/**
 * Update station tutorial overlay.
 * Called once per frame from station-scene.ts.
 */
export function updateStationTutorial(
  state: GameState,
  store: GameStore,
  tutorial: TutorialController,
  dt: number,
  ...
  const isPanelScreen = checkIsPanelScreen(state.screen as StationScreen);
